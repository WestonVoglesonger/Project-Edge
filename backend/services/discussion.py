...
class DiscussionService:
    ...
    def create_discussion(
        self, discussion_data: DiscussionCreate
    ) -> DiscussionResponse:
        if (
            self.db.query(UserEntity)
            .filter(UserEntity.id == discussion_data.author_id)
            .count()
            == 0
        ):
            raise UserNotFoundException(discussion_data.author_id)
    ...
    def update_discussion(
        self, discussion_id: int, discussion_update: DiscussionUpdate
    ) -> DiscussionResponse:
        logger.info(f"Starting update for discussion with id {discussion_id}")

        try:
            discussion_entity = (
                self.db.query(DiscussionEntity)
                .filter(DiscussionEntity.id == discussion_id)
                .one()
            )
            logger.info(f"Found discussion with id {discussion_id}")
        except NoResultFound:
            logger.error(f"Discussion with id {discussion_id} not found")
            raise DiscussionNotFoundException(
                f"Discussion with id {discussion_id} not found"
            )
        ...
        self.db.commit()
        logger.info(
            f"Discussion with id {discussion_id} successfully updated in the database"
        )
    ...
    def get_discussions_by_user(self, author_id: int) -> List[DiscussionResponse]:
        discussions = (
            self.db.query(DiscussionEntity).filter_by(author_id=author_id).all()
        )
        return [discussion.to_discussion_response() for discussion in discussions]
