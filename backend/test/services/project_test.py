...
def test_get_projects_by_user(project_svc: ProjectService):
    # Ensure user2.id is not None with a default value
    user_id = (
        user2.id or 2
    )  # Use 2 as fallback, which appears to be user2's expected ID
    projects = project_svc.get_projects_by_user(user_id)
    assert len(projects) == 1
    assert projects[0].name == project.name
    assert projects[0].description == project.description


def test_update_project(project_svc: ProjectService):
    created_project = project_svc.create_project(project)
    updated_project_data = project_svc.update_project(
        created_project.id, updated_project, current_user_id=2
    )
...
def test_update_project_remove_project_leaders(project_svc: ProjectService):
    ...
    with pytest.raises(ValueError):
        project_svc.update_project(
            created_project.id, updated_project_2, current_user_id=2
        )
...
def test_update_project_user_not_authorized(project_svc: ProjectService):
    ...
    with pytest.raises(UnauthorizedException):
        project_svc.update_project(
            created_project.id, updated_project, current_user_id=1
        )
...
def test_update_project_user_not_found(project_svc: ProjectService):
    ...
    with pytest.raises(UnauthorizedException):
        project_svc.update_project(
            created_project.id, updated_project, current_user_id=999
        )
